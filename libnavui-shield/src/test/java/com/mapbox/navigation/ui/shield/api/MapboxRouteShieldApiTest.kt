...
class MapboxRouteShieldApiTest {
    ...
    @Before
    fun setUp() {
        mockkObject(RoadShieldContentManager)
    }

    @After
    fun tearDown() {
        unmockkObject(RoadShieldContentManager)
    }
    ...
    fun `when get shields for all empty legacy urls`() {
        ...
        coEvery {
            RoadShieldContentManager.getShields(routeShieldToDownload)
        } returns emptyList()
    ...
    fun `when get shields for all legacy urls with all shields`() =
        coroutineRule.runBlockingTest {
            ...
            coEvery { RoadShieldContentManager.getShields(any()) } returns mockResultList
            val callback: RouteShieldCallback = mockk(relaxed = true)
            val shieldSlot = slot<List<Expected<RouteShieldError, RouteShieldResult>>>()
