...
class MapboxManeuverApiTest {
    ...
    fun `when legacy roadShields is invoked returns success`() = coroutineRule.runBlockingTest {
        ...
        coEvery { routeShieldApi.getRouteShieldsFromModels(any()) } returns listOf(
            ExpectedFactory.createError(RouteShieldError("https://mapbox.com/url1", "whatever1")),
            ExpectedFactory.createError(RouteShieldError("https://mapbox.com/url2", "whatever2"))
        )
        ...
        coroutineRule.runBlockingTest {
            ...
            coEvery { routeShieldApi.getRouteShieldsFromModels(any()) } returns listOf(
                ExpectedFactory.createError(
                    RouteShieldError("https://mapbox.com/url1", "whatever1")
                ),
                ExpectedFactory.createError(
                    RouteShieldError("https://mapbox.com/url2", "whatever2")
                )
            )
            val maneuverList = listOf(testManeuvers[0])
    ...
    fun `when mapbox roadShields is invoked returns success`() = coroutineRule.runBlockingTest {
        ...
        coEvery { routeShieldApi.getRouteShieldsFromModels(any()) } returns listOf(
            ExpectedFactory.createValue(
                RouteShieldResult(
                    shield = mockk<RouteShield.MapboxDesignedShield> {
            ...
            coEvery { routeShieldApi.getRouteShieldsFromModels(any()) } returns listOf(
                ...
                ExpectedFactory.createValue(
                    RouteShieldResult(
                        ...
                        origin = RouteShieldOrigin(
                            isFallback = false,
                            originalUrl = "https://shield.mapbox.com/url2",
                            originalErrorMessage = ""
                        )
                    )
                )
